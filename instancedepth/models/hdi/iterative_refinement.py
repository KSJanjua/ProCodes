...
class RefinementTrace:
    ...
    bins: List[torch.Tensor] = field(default_factory=list)     # [S_0, S_1, S_2]
...
class IterativeBinRefinement(nn.Module):
    ...
    def forward(self, features: DepthRangeFeatures) -> RefinementTrace:
        ...
        for i in range(3):
            ...
            c_i = self.confidence_heads[i](f_i, d_upsampled)       # Eq. 1: (B, rd, H_i, W_i)
            s_i = self.bin_heads[i](f_i)                            # ordinal bins, same shape
            trace.bins.append(s_i)
